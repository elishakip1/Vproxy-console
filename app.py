...
def login():
    ...
    if request.method == 'POST':
        ...
        if user_to_login and user_to_login.password == password:
            ...
            add_log_entry("INFO", f"User '{username}' logged in successfully.")
            return redirect(next_page or (url_for('admin') if current_user.is_admin else url_for('index')))
        else:
            logger.warning(f"Failed login attempt for username: '{username}'")
            add_log_entry("WARNING", f"Failed login attempt for username: '{username}'")
...
def logout():
    ...
    add_log_entry("INFO", f"User '{username}' logged out.")
...
def index():
    ...
    if request.method == "POST":
        ...
        if good_count_final > 0 or used_count_final > 0:
            ...
            add_log_entry("INFO", f"Checker run by {current_user.username}. Found {good_count_final} usable proxies.")
        else:
             base_message = f"⚠️ Checked {checks_attempted} proxies ({input_count} submitted{format_warning}). No new usable proxies found."
             add_log_entry("WARNING", f"Checker run by {current_user.username}. Found no usable proxies in batch.")
...
def track_used():
    ...
    try:
        ...
        if add_used_ip(ip, proxy_line):
            ...
            add_log_entry("INFO", f"Proxy marked as used by {current_user.username}: {ip}")
            return jsonify({"status": "success"})
        else:
            # add_used_ip logs its own errors
            logger.error(f"Failed to add used IP/Proxy to Google Sheet: {proxy_line} (IP: {ip})")
            add_log_entry("ERROR", f"Failed to log used proxy {ip} for {current_user.username}.")
        ...
        add_log_entry("ERROR", f"Exception in track-used for {proxy_line}: {str(e)[:50]}")
...
def admin():
    ...
    try:
        ...
        add_log_entry("ERROR", f"Failed to load admin panel: {str(e)[:50]}")
...
def admin_logs():
    ...
    try:
        ...
        add_log_entry("INFO", f"Admin '{current_user.username}' viewed system logs.")
        ...
        add_log_entry("CRITICAL", f"Failed to load system logs: {str(e)[:50]}")
    ...
    if request.method == "POST":
        ...
        if not error_msg:
            ...
            if success_count == len(settings_to_update): 
                ...
                add_log_entry("INFO", f"Admin '{current_user.username}' updated settings.")
                ...
                add_log_entry("ERROR", f"Admin '{current_user.username}' failed to update {len(settings_to_update) - success_count} settings.")
    ...
    try:
        if "save_announcement" in request.form:
            ...
            if update_setting("ANNOUNCEMENT", text): 
                ...
                add_log_entry("INFO", f"Admin '{current_user.username}' updated announcement.")
            else: 
                flash("Error saving announcement to Google Sheet. Check logs.", "danger"); 
                logger.error("Failed to save announcement to GSheet.")
                add_log_entry("ERROR", f"Admin '{current_user.username}' failed to save announcement.")
            ...
            if update_setting("ANNOUNCEMENT", ""): 
                ...
                add_log_entry("INFO", f"Admin '{current_user.username}' cleared announcement.")
            else: 
                flash("Error clearing announcement in Google Sheet. Check logs.", "danger"); 
                logger.error("Failed to clear announcement in GSheet.")
                add_log_entry("ERROR", f"Admin '{current_user.username}' failed to clear announcement.")
    except Exception as e: 
        logger.error(f"Error in admin_announcement route: {e}", exc_info=True); 
        add_log_entry("ERROR", f"Exception in admin_announcement: {str(e)[:50]}")
...
def delete_used_ip_route(ip):
    ...
    try:
        if delete_used_ip(ip): 
            ...
            add_log_entry("INFO", f"Admin '{current_user.username}' deleted used IP: {ip}")
        else: 
            flash(f"Could not delete record for IP {ip}. It might not exist or an error occurred.", "warning") # delete_used_ip logs specifics
            add_log_entry("WARNING", f"Admin '{current_user.username}' failed to delete IP: {ip}")
    except Exception as e: 
        logger.error(f"Unexpected error in delete_used_ip_route for IP {ip}: {e}", exc_info=True); 
        add_log_entry("ERROR", f"Exception deleting used IP {ip}: {str(e)[:50]}")
...
def internal_server_error(e):
    ...
    add_log_entry("CRITICAL", f"500 Server Error at {request.path}: {str(e)[:50]}")
